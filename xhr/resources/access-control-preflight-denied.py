def main(request, response):
    def fail(message):
        response.content = u"FAIL: " + str(message)
        response.status = 400
    ...
    if command == b"reset":
        if request.method == u"GET":
            resetState(token)
            response.content = u"Server state reset"
        else:
            fail(b"Invalid Method.")
    elif state == b"Uninitialized":
        if request.method == u"OPTIONS":
            response.content = u"This request should not be displayed."
        ...
        if request.method == u"GET" and command == b"complete":
            resetState(token)
            response.content = u"Request successfully blocked."
        else:
