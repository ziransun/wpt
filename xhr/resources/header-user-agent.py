def main(request, response):
    ...
    if request.method == u"OPTIONS":
        if not request.headers.get(b"User-Agent"):
            response.content = u"FAIL: User-Agent header missing in preflight request."
            response.status = 400
    else:
        if request.headers.get(b"User-Agent"):
            response.content = u"PASS"
        else:
            response.content = u"FAIL: User-Agent header missing in request"
            response.status = 400
