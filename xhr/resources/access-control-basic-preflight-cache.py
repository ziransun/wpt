def main(request, response):
    def fail(message):
        response.content = u"FAIL " + request.method + u": " + str(message)
        response.status = 400
    ...
    if state == b"Uninitialized":
        ...
        if request.method == u"PUT":
            response.content = u"PASS: First PUT request."
            setState(b"FirstPUTSent", token)
        else:
            fail(state)
    elif state == b"FirstPUTSent":
        if request.method == u"PUT":
            response.content = u"PASS: Second PUT request. Preflight worked."
