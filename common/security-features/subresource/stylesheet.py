...
def generate_payload(request, server_data):
    ...
    if type == b'image':
        return subresource.get_template(u"image.css.template") % {u"id": isomorphic_decode(request.GET["id"])}

    elif type == b'font':
        return subresource.get_template(u"font.css.template") % {u"id": isomorphic_decode(request.GET["id"])}

    elif type == b'svg':
        return subresource.get_template(u"svg.css.template") % {
            u"id": isomorphic_decode(request.GET["id"]),
            u"property": isomorphic_decode(request.GET["property"])}
