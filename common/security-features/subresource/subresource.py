...
def get_template(template_basename):
    ...
    with open(template_filename, u"r") as f:
        return f.read()
...
def redirect(url, response):
    ...
    response.writer.write_header(b"location", url)
...
def preprocess_stash_action(request, response):
    ...
    path = request.GET.get(u"path", request.url.split(u'?'))[0]

    if action == "put":
        value = request.GET[b"value"]
