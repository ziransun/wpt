...
def download_artifacts(artifacts):
    ...
    for artifact in artifacts:
        ...
        for candidate in artifacts_data["artifacts"]:
            print("DEBUG: candidate: %s pattern: %s" % (candidate["name"], artifact["artifact"]))
            if fnmatch.fnmatch(candidate["name"], artifact["artifact"]):
                found = True
                ...
                if artifact.get("extract"):
                    unpack(dest_path)
        if not found:
            print("WARNING: No artifact found matching %s in task %s" % (artifact["artifact"], artifact["task"]))
