...
def create_wav_header(sample_rate, bit_depth, channels, duration):
    ...
    data += b'RIFF'
    # ChunkSize
    data += struct.pack(u'<L', 36 + sub_chunk_2_size)
    # Format
    data += b'WAVE'
    # Subchunk1ID
    data += b'fmt '
...
def main(request, response):
    ...
    range_header_match = range_header and re.search(r'^bytes=(\d*)-(\d*)$', range_header.decode("iso-8859-1"))
    #range_header_match = range_header and re.search(rb'^bytes=(\d*)-(\d*)$', range_header)
    ...
    if range_received_key and range_header:
        ...
        request.server.stash.take(range_received_key, u'/fetch/range/')  #ziran
        # This is later collected using stash-take.py
        request.server.stash.put(range_received_key, u'range-header-received', u'/fetch/range/') #ziran
    ...
    if accept_encoding_key:
        ...
        request.server.stash.put(
            accept_encoding_key,
            request.headers.get(b'Accept-Encoding', b''),
            u'/fetch/range/'
        )
    ...
    if range_header_match:
        ...
        response.headers.set(b"Content-Range", content_range.encode("iso-8859-1"))
    else:
